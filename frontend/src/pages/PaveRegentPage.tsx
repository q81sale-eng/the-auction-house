...
export const PaveRegentPage: React.FC = () => {
  const { tr, lang } = useT();
  const t = tr.pave.regent;
    ...
    <Layout>
      ...
      <section className="min-h-screen flex flex-col justify-end bg-obsidian-950 relative overflow-hidden px-4 pb-20">
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none">
          <span className="font-serif text-[clamp(18rem,50vw,36rem)] text-obsidian-900 leading-none">R</span>
        </div>

        {/* Thin gold line across viewport */}
              ...
              <article key={piece.ref} className="border border-obsidian-800 hover:border-gold-500/30 transition-colors duration-500">
                ...
                <div className="aspect-square bg-obsidian-900 flex items-center justify-center relative overflow-hidden">
                  ...
                  <div className="relative z-10 text-center">
                    <p className="font-serif text-5xl text-white/10 select-none">PAVÉ</p>
                    <p className="font-mono text-gold-500/30 text-xs mt-2 tracking-[0.3em]">{piece.ref}</p>
                  </div>
                  {/* Corner accent */}
                  <div className="absolute top-4 start-4 w-6 h-6 border-t border-s border-gold-500/30" />
                  <div className="absolute bottom-4 end-4 w-6 h-6 border-b border-e border-gold-500/30" />
                </div>
                ...
                <div className="p-8">
                  ...
                  <div className="flex items-end justify-between mb-3">
                    ...
                    <a href="#regent-inquiry" className="text-obsidian-500 hover:text-gold-500 text-xs uppercase tracking-[0.2em] transition-colors">
                      {t.inquire} →
                    </a>
                  </div>
                  <WhatsAppShare title={`Pavé Regent — ${piece.name}`} lang={lang} />
                </div>
              </article>
            ))}
          </div>
      ...
      {/* ── Inquiry ───────────────────────────────────────────────────────── */}
      <section id="regent-inquiry" className="bg-obsidian-900 border-t border-obsidian-800 py-24 px-4">
